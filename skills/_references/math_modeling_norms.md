...
- 不要出现图、表、代码块或复杂推导公式；不要只重述题目；不要用"我们认为……""本文将……"等空泛表述。
...
引言应说明研究意义，并用真实文献交代相关研究或方法现状；结尾应逐题回答并在适用时提出有结果依据的建议。比赛模板允许把这些内容放入等价章节，不要求机械套用固定标题。

### 标题、关键词与自动编号

- 标题应反映研究对象、核心方法或主要目标，避免逐字复制赛题标题。
- CUMCM 写作质量建议采用 3--5 个关键词并用分号分隔；关键词描述对象、模型或方法，不把 Python、MATLAB、Excel 等软件名当作关键词。该建议不替代当期官方通知。
- 图、表和需要引用的行间公式使用排版引擎自动编号与标签，正文通过交叉引用调用；caption 和正文源文件不得手写易漂移的编号。
- 正文只保留必要的伪代码或短代码片段，完整程序和大段源码放入附录或支撑材料。
