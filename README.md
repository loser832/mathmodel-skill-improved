...
这是一个面向数学建模竞赛的 Agent skills 工作流改进版，重点增强了偏好继承、跨设备中断接续、图片路径约定、验收返工、按需多 agent 协作，以及有界的结构化交叉质询。
...
- `skills/1start-mathmodel/references/resume.md`：项目进度保存、旧项目恢复和跨设备交接协议。
...
`skills/` 是版本控制和跨设备同步使用的维护源；修改应先落在这里，再将其复制到 Codex/Agent 的个人 skills 目录作为本机运行副本。Windows PowerShell 示例：
